import { RESULT_TAG_DEFAULT } from "constants/search-settings"
  ...
  describe("With a multitude of vaguely finance-related notes", () => {
    ...
    beforeEach(() => {
      ...
      notes = noteCandidateData.map((noteData, index) => {
        const tags = noteData[2] ? [ noteData[2] ] : [];
    ...
    it("should find notes related to long-term planning", async () => {
      ...
      const result = await searchAgent.search(userQuery);
      // "gpt-5.1 result: Net Worth & Retirement Estimates" excluded for having the RESULT_TAG_DEFAULT tag
      const expectedNames = [ "Escaping the rat race: A bangin plan", "Working Toward Retirement Projections" ];
