...
export async function callRemoteAI(plugin, app, model, messages, promptKey, allowResponse, modelsQueried = []) {
  ...
  try {
    const providerApiKey = apiKeyFromApp(app, providerEm);
...
export async function llmPrompt(app, prompt, plugin, { aiModel = null, concurrency = 1, jsonResponse = false, } = {}) {
  let modelCandidates = plugin.preferredModels();
  if (aiModel) {
  ...
  try {
    ...
    const providerEm = providerFromModel(modelToUse);
    const providerApiKey = apiKeyFromApp(app, providerEm);
