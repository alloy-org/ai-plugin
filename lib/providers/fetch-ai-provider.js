import { defaultProviderModel, providerFromModel, providerNameFromProviderEm, providerEndpointUrl, apiKeyFromApp } from "providers/ai-provider-settings"
import { jsonResponseFromStreamChunk, responseFromStreamOrChunk, shouldStream, streamPrefaceString } from "providers/fetch-json"
...
// @param {boolean} jsonResponseExpected - Whether a JSON response is expected from the model
...
function requestBodyForProvider(jsonResponseExpected, messages, model, promptKey, stream, tools) {
  let body;
  const providerEm = providerFromModel(model);
...
async function makeRequest(apiKey, attemptNumber, jsonResponseExpected, messages, model, promptKey, stream, timeoutSeconds, tools = null) {
  const providerEm = providerFromModel(model);
  if (attemptNumber > 0) console.debug(`Attempt ${ attemptNumber + 1 }: Trying ${ model } with ${ promptKey }`);

  const body = requestBodyForProvider(jsonResponseExpected, messages, model, promptKey, stream, tools);
...
async function requestWithRetry(app, model, messages, tools, apiKey, promptKey, streamCallback, allowResponse, {
    suppressParallel = false, retries = 3, timeoutSeconds = 30 } = {}) {
  let error, response;
  const providerEm = providerFromModel(model);
  const providerName = providerNameFromProviderEm(providerEm);
  ...
  if (suppressParallel) {
    ...
    for (let i = 0; i < retries; i++) {
      try {
        response = await makeRequest(apiKey, i, jsonResponseExpected, messages, model, promptKey, stream, timeoutSeconds, tools);
    ...
    const promises = Array.from({ length: retries }, (_, i) =>
      makeRequest(apiKey, i, jsonResponseExpected, messages, model, promptKey, stream, timeoutSeconds, tools).catch(e => {
