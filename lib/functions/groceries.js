...
// --------------------------------------------------------------------------
export async function groceryContentFromJsonOrText(plugin, app, noteUUID, groceryArray, aiModels, promptResponse) {
  const jsonModels = await aiModels(plugin, app, "sortGroceriesJson");
  if (jsonModels.length) {
    const confirmation = groceryCountJsonConfirmation.bind(null, groceryArray.length);
    const jsonGroceries = await promptResponse(plugin, app, noteUUID, "sortGroceriesJson", { groceryArray },
      { allowResponse: confirmation });
    if (typeof(jsonGroceries) === "object") {
      return noteContentFromGroceryJsonResponse(jsonGroceries)
    }
  } else {
    const sortedListContent = await promptResponse(plugin, app, noteUUID, "sortGroceriesText", { groceryArray },
