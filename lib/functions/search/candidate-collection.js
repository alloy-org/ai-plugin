...
async function searchNotesByStrategy(searchAgent, primaryKeywords, secondaryKeywords, tagRequirement) {
  ...
  return await executeSearchStrategy(searchAgent, primaryKeywords, secondaryKeywordsToUse, tagRequirement);
}
...
async function executeSearchStrategy(searchAgent, primaryKeywords, secondaryKeywords, tagRequirement) {
  ...
  const candidatesByUuid = new Map(); // uuid -> note (augmented with matchCount)
  ...
  if (candidatesByUuid.size < MIN_TARGET_RESULTS && secondaryQueries.length) {
    console.log(`[${ strategyName }] Below minimum target (${ MIN_TARGET_RESULTS }), broadening filterNotes with secondary keywords`);
  ...
  if (candidatesByUuid.size < MIN_FILTER_NOTES_RESULTS) {
    console.log(`[${ strategyName }] filterNotes candidates (${ candidatesByUuid.size }) below minimum (${ MIN_FILTER_NOTES_RESULTS }), supplementing with app.searchNotes`);
    await addMatchesFromSearchNotes(searchAgent, candidatesByUuid, primaryQueries, maxResultsPerQuery);

    if (candidatesByUuid.size < MIN_FILTER_NOTES_RESULTS && secondaryQueries.length) {
...
//
// @param {Map<string, Object>} candidatesByUuid - Map of uuid -> note
// @param {Object} note - Note candidate (must include uuid)
...
//
// @param {SearchAgent} searchAgent - The search agent instance with app API access
// @param {Map<string, Object>} candidatesByUuid - Candidate accumulator
