...
//
// @param {string} searchAttempt - The search attempt strategy constant
// @returns {string} Human-readable strategy name
function getStrategyName(searchAttempt) {
  if (searchAttempt === ATTEMPT_FIRST_PASS) return "First Pass";
  if (searchAttempt === ATTEMPT_KEYWORD_PAIRS) return "Keyword Pairs";
  if (searchAttempt === ATTEMPT_INDIVIDUAL) return "Individual Keywords";
  return "Unknown";
}
