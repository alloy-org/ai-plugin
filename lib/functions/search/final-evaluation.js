...
export async function phase4_scoreAndRank(searchAgent, analyzedCandidates, criteria, userQuery) {
  // Filter out notes that have already been rated in previous search attempts
  ...
  // Record rated UUIDs so they are excluded in subsequent search attempts
...
`;
  ...
  if (sanityCheck.suggestAction === "retry_broader") {
    ...
    // Increment retryCount only for non-retry_broader paths (retry_broader increments in nextSearchAttempt)
...
// Timeout errors are retried up to MAX_PHASE4_TIMEOUT_RETRIES times.
