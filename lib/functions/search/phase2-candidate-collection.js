...
// --------------------------------------------------------------------------
// Prepare keywords for searching based on strategy and previously maxed-out keywords.
// Returns keywords ready for querying (strategy-transformed and filtered).
//
// @param {Array<string>} primaryKeywords - Full list of primary keywords
// @param {SearchAgent} searchAgent - The search agent instance
// @param {Array<string>} secondaryKeywords - Full list of secondary keywords
// @returns {{ activePrimaryKeywords: Array<string>, activeSecondaryKeywords: Array<string> }}
function activeKeywordsForStrategy(primaryKeywords, searchAgent, secondaryKeywords) {
  ...
  // Filter out keywords that maxed out in previous search attempts
...
// --------------------------------------------------------------------------
// Collect candidates using filterNotes and searchNotes fallback.
// Accumulates candidates into candidatesByUuid and records maxed-out keywords.
//
// @param {Map<string, SearchCandidateNote>} candidatesByUuid - Candidate accumulator
// @param {number} minTargetResults - Minimum candidates to aim for
// @param {Array<string>} primaryKeywords - Active primary keywords for querying
// @param {SearchAgent} searchAgent - The search agent instance
// @param {Array<string>} secondaryKeywords - Active secondary keywords for querying
// @param {Object} tagRequirement - Tag filtering requirements
async function collectSearchCandidates(candidatesByUuid, minTargetResults, primaryKeywords, searchAgent, secondaryKeywords, tagRequirement) {
  ...
  // 2) filterNotes secondary keywords (only if we need to broaden)
  ...
  if (candidatesByUuid.size < minTargetResults && activeSecondaryKeywords.length) {
    ...
    maxedOutSecondaryKeywords = await collectCandidatesWithKeywordLimits(candidatesByUuid, false, activeSecondaryKeywords, 
      searchAgent, tagRequirement);
  ...
  // Record maxed-out keywords for subsequent search attempts
  ...
  // 3) searchNotes fallback if filterNotes produced too few candidates
...
// --------------------------------------------------------------------------
// Calculate keyword density estimates and sort candidates by relevance.
// Fetches note content for top candidates, calculates density, and sorts.
//
// @param {Array<SearchCandidateNote>} candidates - Candidates to process
// @param {Array<string>} primaryKeywords - Full primary keyword list for density calculation
// @param {SearchAgent} searchAgent - The search agent instance
// @param {Array<string>} secondaryKeywords - Full secondary keyword list for density calculation
// @returns {Promise<Array<SearchCandidateNote>>} Sorted candidates with density estimates
async function densitySortedCandidates(candidates, primaryKeywords, searchAgent, secondaryKeywords) {
  const strategyName = searchAgent.searchAttempt;

  // Sort by preContentMatchScore and limit before fetching body content (expensive)
  const sortedByPreContentScore = candidates
  ...
  // Fetch content and calculate keyword density estimates
  ...
  // Sort by keyword density, with recency as tiebreaker
...
// --------------------------------------------------------------------------
// Execute the search strategy to collect and rank candidates.
// Coordinates keyword preparation, candidate collection, and density sorting.
//
// @param {Array<string>} primaryKeywords - Full list of primary keywords
// @param {number} resultCount - Number of results requested by user
// @param {SearchAgent} searchAgent - The search agent instance with app API access
// @param {Array<string>} secondaryKeywords - Full list of secondary keywords
// @param {Object} tagRequirement - Tag filtering requirements
// @returns {Promise<Array<SearchCandidateNote>>} Sorted candidates with density estimates
async function executeSearchStrategy(primaryKeywords, resultCount, searchAgent, secondaryKeywords, tagRequirement) {
  ...
  // Step 1: Prepare keywords based on strategy and previous maxed-out keywords
  ...
  // Step 2: Collect candidates using filterNotes and searchNotes
  const candidatesByUuid = new Map();
  await collectSearchCandidates(candidatesByUuid, minTargetResults, activePrimaryKeywords, 
    searchAgent, activeSecondaryKeywords, tagRequirement);

  // Step 3: Calculate density and sort (using full keyword lists for accurate density)
