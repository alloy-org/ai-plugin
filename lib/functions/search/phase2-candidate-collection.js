...
async function upsertCandidate(candidatesByUuid, noteHandle, searchAgent, increment = 1) {
  ...
  console.log(`Created searchCandidate with ${ noteContent.length }-length content in ${ new Date() - startAt }ms ending ${ new Date() } from`, Object.fromEntries(["name", "content", "url"].map(k => [ k, noteHandle[k] ])), "and typeof content", typeof noteHandle.content);
