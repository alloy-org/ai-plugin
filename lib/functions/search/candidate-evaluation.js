...
async function analyzeNoteDeep(note, searchAgent, searchParams) {
  ...
  const needAttachments = searchParams.criteria.containsPDF;
  const needImages = searchParams.criteria.containsImage;
  const needContent = searchParams.exactPhrase || searchParams.criteria.containsURL;
  ...
  console.log(`Deep analysis finds note "${ note.name }" from ${ JSON.stringify(criteria) } finds needAttachments: ${ checks.hasPDF }, needImages: ${ checks.hasImage }, needContent: ${ content ? "fetched" : "not fetched" }`);
