...
`;
  ...
  // Todo: BodyRelevance seems at least as important as titleRelevance, and can be distinct from keywordDensity?
  // Todo: Is criteriaMarch composite or binary? Only seems composite if we expect users will say they _prefer_ notes with attachments, but that's not captured in UserCriteria yet so
  const weights = {
    titleRelevance: 0.28,
    keywordDensity: 0.23,
    criteriaMatch: 0.20,
  ...
  console.log(`Finsihed with ${ sortedNotes.length } sorted notes, headlined by "${ sortedNotes[0]?.name }"`);
  return sortedNotes;
}
