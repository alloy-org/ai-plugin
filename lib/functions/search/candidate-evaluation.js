...
export async function phase4_scoreAndRank(searchAgent, analyzedCandidates, criteria, userQuery) {
  ...
  console.log(`[Phase 4] ${ analyzedCandidate.length > MAX_NOTES_PER_RANKING ? "Split" : "" } ${ analyzedCandidates.length } candidates into ${ batches.length } batches for final scoring`);
