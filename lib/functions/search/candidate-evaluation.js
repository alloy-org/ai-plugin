...
export async function phase3_deepAnalysis(searchAgent, candidates, criteria) {
  ...
  const deepAnalysis = await searchAgent.parallelLimit(
    topCandidates.map(note => () => analyzeNoteDeep(searchAgent, note, criteria)),
...
async function analyzeNoteDeep(searchAgent, note, criteria) {
  const checks = {};
  let content = null;
...
function rankPreliminary(candidates, criteria) {
