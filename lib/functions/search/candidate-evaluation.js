...
const LLM_SCORE_BODY_CONTENT_LENGTH = 2000;
...
5. RECENCY: If the user specified recency requirement, does it meet that? If not, score 10 for recency within the past month, and scale down to 0 for over 12 months old.
...
${ index }. "${ candidate.note.name }"
   ...
   Body Content: ${ candidate.content?.slice(0, LLM_SCORE_BODY_CONTENT_LENGTH) }
...
// @returns {Promise<Object>} The final search result with found notes, or triggers a retry
