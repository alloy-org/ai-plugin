...
// --------------------------------------------------------------------------------------
async function sizeModelFromUser(plugin, app) {
...
async function imageMarkdownFromPrompt(plugin, app, prompt, apiKey, { note = null } = {}) {
  ...
  const [ size, model ] = await sizeModelFromUser(plugin, app)
  ...
  const response = await fetch("https://api.openai.com/v1/images/generations", {
    ...
    body: JSON.stringify({
      prompt,
      model,
      n: model === "dall-e-2" ? 3 : 1, // As of Dec 2023, v3 can only generate one image per run
      size,
    })
  });
