...
`;
  ...
  async searchTitlesByStrategy(primaryKeywords, tagRequirement) {
    if (primaryKeywords.length === 0) return [];

    if (this.searchAttempt === SearchAgent.ATTEMPT_FIRST_PASS) {
      // First pass: Search all keywords together
      const titleQuery = primaryKeywords.join(" ");
      const results = await this.app.filterNotes({
        query: titleQuery,
    ...
    } else if (this.searchAttempt === SearchAgent.ATTEMPT_KEYWORD_PAIRS) {
    ...
    } else if (this.searchAttempt === SearchAgent.ATTEMPT_INDIVIDUAL) {
  ...
  async phase2_collectCandidates(criteria) {
    ...
    const titleCandidates = await this.searchTitlesByStrategy(primaryKeywords, tagRequirement);
