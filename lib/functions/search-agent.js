...
export default class SearchAgent {
  ...
  constructor(app, plugin) {
    ...
    this.llm = llmWithSearchPreference.bind(this, app, plugin); // Function that takes prompt, returns parsed JSON
  ...
  async llmWithRetry(prompt, validateFn, options = {}) {
    const models = modelsToTryFromPreference(this.app, this.preferredAiModel);
...
async function llmWithSearchPreference(app, plugin, prompt, options = {}) {
  const mergedOptions = { ...options };
  const aiModelExplicitlyProvided = Object.prototype.hasOwnProperty.call(mergedOptions, "aiModel");

  if (!aiModelExplicitlyProvided || mergedOptions.aiModel === undefined || mergedOptions.aiModel === null) {
    if (this.preferredAiModel) {
      mergedOptions.aiModel = this.preferredAiModel;
    }
  }

  const result = await llmPrompt(app, plugin, prompt, mergedOptions);
...
// --------------------------------------------------------------------------
// @param {object} app - Amplenote app object
// @param {string|null} preferredAiModel - Preferred model to try first
// @returns {string[]} Ordered models list to try
function modelsToTryFromPreference(app, preferredAiModel) {
  const models = preferredModels(app) || [];
  if (!preferredAiModel) return models;

  const withoutPreferred = models.filter(model => model !== preferredAiModel);
  return [ preferredAiModel, ...withoutPreferred ];
}
