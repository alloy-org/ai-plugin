...
const PROGRESS_RETRY_RECEIVED_INPUT = 60;
const PROGRESS_RETRY_PHASE1_COMPLETE = 65;
const PROGRESS_RETRY_PHASE2_COMPLETE = 70;
const PROGRESS_RETRY_PHASE3_COMPLETE = 75;
...
export default class SearchAgent {
  ...
  async search(userQuery, { criteria = {}, options = {} } = {}) {
    try {
      ...
      // Create summary note with results (skip during retry passes - the first pass will create it after merging results from passes)
  ...
  _mergeRankedNotes(previousPassNotes, retryPassNotes) {
    ...
    // Add previous pass notes first (they have already been scored)
    for (const note of previousPassNotes) {
      notesByUuid.set(note.uuid, note);
    }

    // Add retry pass notes (duplicates should already be excluded by ratedNoteUuids)
    ...
    // Convert to array and sort by finalScore descending
  ...
  async nextSearchAttempt(userQuery, criteria, previousPassRankedNotes = []) {
    ...
    // Merge previous pass notes with retry results
