...
class SearchAgent {
  ...
  constructor(app, llmProvider) {
    this.app = app;
    this.llm = llmProvider; // Function that takes prompt, returns parsed JSON
    this.retryCount = 0;
    this.maxRetries = 2;
...
`;
    ...
    // Apply any manual overrides from options
    return {
      ...extracted,
      ...options,
...
${i}. "${c.note.name}"
   UUID: ${c.note.uuid}
   Tags: ${c.note.tags?.join(", ") || "none"}
   Updated: ${c.note.updated}
   ${c.contentPreview ? `Content preview: ${c.contentPreview}` : "No content fetched"}
