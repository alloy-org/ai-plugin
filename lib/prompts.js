...
function messageArrayFromPrompt(promptKey, promptParams) {
  const userPrompts = {
    ...
    sortGroceries: ({ noteContent }) => ([`Sort the following markdown-formatted list of groceries by what aisle it will be found in the grocery store:`,
      noteContent,
      `Prefix each grocery aisle (each task section) with a "# ". For example, "# Produce\n[ ] Bananas\n[ ] Grapes\n\n# Bakery\n[ ] Donuts\n[ ] Bread\n\n# Seafood\n[ ] Salmon fillets"`,
      `DO NOT RESPOND WITH ANY EXPLANATION, only groceries and aisles.` ]),
