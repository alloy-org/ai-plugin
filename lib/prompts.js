import { truncate } from "app-util"
import { REJECTED_RESPONSE_PREFIX } from "constants/functionality"
import { OLLAMA_TOKEN_CHARACTER_LIMIT } from "constants/provider"
import { PLUGIN_NAME } from "constants/settings"
import { isModelOllama, modelTokenLimit } from "providers/ai-provider-settings"
...
export async function contentfulPromptParams(app, noteUUID, promptKey, promptKeyParams, aiModel,
  ...
  // For continue/complete actions, allow up to 10k characters for better context
  // For other actions, limit to 1k to leave room for prompts and rejected responses
  const noteContentCharacterLimit = Math.min(inputLimit * 0.5, longContent ? 10000 : 1000);
...
function userPromptFromPromptKey(promptKey, promptParams) {
  ...
  if ([ "continue", "insertTextComplete", "replaceTextComplete" ].find(key => key === promptKey)) {
    ...
    if (promptKey === "replaceTextComplete") {
      ...
      // Use regex to handle potential whitespace around the plugin name and action
    ...
    // Add example for continue/complete actions to guide the LLM
    if (!suppressExample) {
