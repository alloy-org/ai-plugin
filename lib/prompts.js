...
const SYSTEM_PROMPTS = {
  ...
  rhyming: "You are a helpful rhyming word generator that responds with a list of rhyming words in a JSON object",
...
function messageArrayFromPrompt(promptKey, promptParams) {
  const userPrompts = {
    ...
    rhyming: ({ noteContent, rejectedWords, text }) => ([
      JSON.stringify({
        instruction: "Respond with a JSON object containing ONLY ONE KEY called 'rhymes', that contains an array of 10 rhyming words or phrases that were not rejected",
        rejectedWords,
        rhymesWith: text,
        rhymeResponseStringMaxWords: 3,
        rhymeStringsToReturn: 10,
        contextAroundRhymingWord: noteContent.replace(text, `<replace>${ text }</replace>`)
      }),
    ]),
    sortGroceries: ({ groceryArray }) => ([`Sort the following array groceries by what aisle it can be found in the grocery store:`,
      `[ "${ groceryArray.join(`", "`) }" ]`,
      `Prefix each grocery aisle (each task section) with a "# ".\n\nFor example, if the input array were ["salmon fillets", "Bananas", "Donuts", "Grapes", "Bread"] then the correct answer would be "# Produce\n[ ] Bananas\n[ ] Grapes\n\n# Bakery\n[ ] Donuts\n[ ] Bread\n\n# Seafood\n[ ] salmon fillets"`,
