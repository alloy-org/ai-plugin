...
describe("This here plugin", () => {
  ...
  it("should allow user to continue a sentence", async () => {
    const content = `To be, or not to be, that is {${ plugin.constants.pluginName }: Continue}`;
    const { app, note } = mockAppWithContent(content);
    mockAlertAccept(app);
    const ollamaModel = "llama2"
    for (const aiModel of [ ollamaModel, DEFAULT_OPENAI_MODEL ]) {
      app.setSetting(AI_MODEL_LABEL, aiModel);
      console.log("What does", aiModel, "say about", content, "?");
      await plugin.insertText["Continue"](app, content);
      expect(["happiness", "the question"].find(w => note.body.toLowerCase().includes(w))).toBeTruthy();
      note.body = content;
    }
  });
