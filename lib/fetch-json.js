...
async function streamIsomorphicFetch(app, response, aiModel, responseJsonExpected, callback) {
  ...
  let cumulative = "";
  let abort, content, failedParseContent, incrementalContents;
  ...
  await responseBody.on("readable", () => {
    ...
    while (failLoops < 3) {
      ...
      if (chunk) {
        ...
        if (failedParseContent) console.debug("failedParseContent is", failedParseContent);
        if (receivedContent) content = receivedContent;
        if (receivedContent) cumulative += receivedContent;
        if (abort) break;
