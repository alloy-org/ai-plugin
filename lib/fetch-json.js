...
import { STREAM_MOCK_MODEL } from "./constants/provider"
import { AI_MODEL_LABEL, MAX_SPACES_ABORT_RESPONSE } from "./constants/settings"
...
async function responseTextFromStreamResponse(app, response, aiModel, responseJsonExpected, streamCallback) {
  if (typeof(global) !== "undefined" && typeof(global.fetch) !== 'undefined') { // isomorphic-fetch from tests
    if (app.settings[AI_MODEL_LABEL] === STREAM_MOCK_MODEL) {
      return await streamMockedFetch(app, response, aiModel, responseJsonExpected, streamCallback);
    } else {
  ...
  await responseBody.on("readable", () => {
    ...
    while (failLoops < 3) {
      ...
      if (chunk) {
        ...
        ({ abort, incrementalContents, receivedContent } = responseObject);
        console.debug("responseObject content thus far", receivedContent, ". Incrementally:", incrementalContents);
...
// --------------------------------------------------------------------------
async function streamMockedFetch(app, response, aiModel, responseJsonExpected, callback) {
  const responseBody = await response.body;
  let abort, content;
  ...
  await responseBody.on("readable", () => {
    ...
    if (chunk) {
      ...
      for (const streamChunk of chunks) {
        responseObject = callback(app, JSON.stringify(streamChunk), receivedContent, aiModel, responseJsonExpected);
        ({ abort, incrementalContents, receivedContent } = responseObject);
        console.debug("responseObject content thus far", receivedContent, ". Incrementally:", incrementalContents);
        if (receivedContent) content = receivedContent;
        if (abort) break;
...
function shouldContinueStream(chunkStrings, accumulatedResponse) {
  const tooMuchSpace = (chunkStrings?.length && (accumulatedResponse?.length || 0) >= MAX_SPACES_ABORT_RESPONSE &&
    accumulatedResponse.substring(accumulatedResponse.length - MAX_SPACES_ABORT_RESPONSE).trim() === "");
  if (tooMuchSpace) {
