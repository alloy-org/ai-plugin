...
export function jsonResponseFromStreamChunk(supposedlyJsonContent, failedParseContent) {
  ...
  try {
    ...
    jsonResponse = JSON.parse(supposedlyJsonContent.trim());
  } catch(e) {
    console.debug("Failed to parse JSON from", supposedlyJsonContent);
    if (failedParseContent) {
      try {
        jsonResponse = JSON.parse(failedParseContent + supposedlyJsonContent.trim());
      } catch(err) {
        return { failedParseContent: failedParseContent + supposedlyJsonContent };
      }
    } else {
      const jsonStart = supposedlyJsonContent.indexOf("{"); // As of Dec 2023, WBH observes that OpenAI is fond of sending back strings like "data: {\"choices\":[{\"finish_reason\":\"length\"}]}\n\n"
      if (jsonStart) {
        try {
         jsonResponse = JSON.parse(supposedlyJsonContent.substring(jsonStart).trim());
         return { jsonResponse }
        } catch(err) {
          console.debug("Moving start position didn't fix JSON parse error");
        }
      }
      return { failedParseContent: supposedlyJsonContent };
    }
  }
  return { jsonResponse }
}
