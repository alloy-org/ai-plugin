...
export async function responseTextFromStreamResponse(app, response, callback) {
  ...
  while (!error) {
    ...
    if (done || failLoops > 3) {
      console.debug("Completed generating response length", responseText.length);
      break;
    } else if (value) {
      console.debug("Received initial value", value);
      const decodedValue = decoder.decode(value, { stream: true })
      console.debug("Value decoded to", decodedValue);
      const jsonResponse = JSON.parse(decodedValue.trim());
      console.debug("Value JSONified to", jsonResponse);
      const content = jsonResponse?.message?.content;
      try {
        if (typeof (content) === "string") {
          failLoops = 0;
          const abort = callback(content);
          if (abort) break;
        } else {
        ...
        console.error("There was an error parsing the response from Ollama:", error);
        break;
      }
    } else {
      failLoops += 1;
