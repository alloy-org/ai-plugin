  ...
  await responseBody.on("readable", () => {
    ...
    if (chunk) {
      ...
      for (const streamChunk of chunks) {
        ...
        if (receivedContent) content = receivedContent;
        if (abort) break;
        const shouldContinue = shouldContinueStream(incrementalContents, receivedContent);
        if (!shouldContinue) {
          console.debug("shouldContinueStream is false for", incrementalContents, receivedContent)
          break;
        }
      }
    }
