  ...
  await responseBody.on("readable", () => {
    ...
    while (failLoops < 3) {
      ...
      if (chunk) {
        ...
        const iterContent = responseObject?.receivedContent;
        console.debug("responseObject content", iterContent);
        ({ abort, receivedContent } = responseObject);
        if (receivedContent) content = receivedContent;
        if (abort) break;
        if (iterContent?.length && iterContent.trim() === "" && (content?.length || 0) >= MAX_SPACES_ABORT_RESPONSE &&
            content.substring(content.length - MAX_SPACES_ABORT_RESPONSE).trim() === "") {
          console.debug("Exceeded consecutive blank space allocation. Aborting response");
          break;
        }
      } else {
        failLoops += 1;
