...
export function extractJsonFromString(string) {
  ...
  let json;
  try {
    json = JSON.parse(jsonAndAfter.split("\n")[0]);
  } catch(e) {
    console.debug("Failed to parse first line")
    try {
      json = JSON.parse(jsonAndAfter);
    } catch {
      console.debug("Failed to parse all after content")
    }
    if (!json) return null;
  }
...
// --------------------------------------------------------------------------
export async function responseTextFromStreamResponse(app, response, responseJsonExpected, callback) {
    ...
    if (done || failLoops > 3) {
      ...
      try {
        if (typeof (decodedValue) === "string") {
          failLoops = 0;
          const response = callback(app, decodedValue, receivedContent, jsonResponseExpected);
          if (response) {
