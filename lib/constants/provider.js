...
// --------------------------------------------------------------------------
// Get the API endpoint for a given provider and model
// Some providers (like Gemini) include the model name in the URL path
// See: https://ai.google.dev/gemini-api/docs/text-generation#generate-text
export function endpointForProvider(providerEm, model) {
  const endpoint = PROVIDER_ENDPOINTS[providerEm];
  return endpoint.replace('{model-name}', model);
}
