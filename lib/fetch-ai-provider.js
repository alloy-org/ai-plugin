import { defaultProviderModel, PROVIDER_ENDPOINTS } from "./constants/provider"
...
export async function callOpenAI(plugin, app, model, messages, promptKey, allowResponse, modelsQueried = []) {
  ...
  try {
    const providerApiKey = apiKeyFromApp(plugin, app, "openai");
    return await requestWithRetry(app, model, messages, "openai", tools, providerApiKey, promptKey, streamCallback, allowResponse,
