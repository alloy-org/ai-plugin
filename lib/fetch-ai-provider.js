import { defaultProviderModel, PROVIDER_ENDPOINTS, MODELS_PER_PROVIDER, providerNameFromProviderEm } from "./constants/provider"
...
async function requestWithRetry(app, model, messages, providerEm, tools, apiKey, promptKey, streamCallback, allowResponse, {
  ...
  if (response?.ok) {
    ...
    if (result && result.error) {
      app.alert(`Failed to call ${ providerName }: ${ result.error.message }`);
      return null;
    }
  }
}
