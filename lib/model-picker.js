...
import { isJsonEndpoint, promptsFromPromptKey } from "./prompts"
...
export async function sendQuery(plugin, app, promptKey, promptParams, { contentIndex = null, preferredModels = null,
  ...
  for (let i = 0; i < preferredModels.length; i++) {
    ...
    try {
      response = await responseFromPrompts(plugin, app, aiModel, messages, jsonEndpoint, { allowResponse });
    } catch(e) {
...
// --------------------------------------------------------------------------
export function responseFromPrompts(plugin, app, aiModel, messages, jsonEndpoint, { allowResponse = null } = {}) {
  if (isModelOllama(aiModel)) {
    return callOllama(plugin, app, aiModel, messages, jsonEndpoint);
  } else {
    return callOpenAI(plugin, app, aiModel, messages, jsonEndpoint, { allowResponse });
  }
}
