...
async function promptForProviderPrecedence(app) {
  ...
  if (configuredProviderEms.length === 1) {
    // Only one provider, no need to prompt for precedence
    const sortedModels = [PROVIDER_DEFAULT_MODEL[configuredProviderEms[0]]];
  ...
  const sortedModels = providerPrecedence.map(({ providerEm }) => PROVIDER_DEFAULT_MODEL[providerEm]);
