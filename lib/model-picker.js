...
export async function aiModelFromUserIntervention(plugin, app, { defaultProvider = "openai", optionSelected = null } = {}) {
  ...
  // Add checkmarks to providers that are already configured
  const configuredProviders = [];
  for (const option of providerOptions) {
    if (option.value !== "ollama") {
      const settingKey = PROVIDER_SETTING_KEY_LABELS[option.value];
      const minKeyLength = MIN_API_KEY_CHARACTERS[option.value];
      if (app.settings[settingKey]?.trim()?.length >= minKeyLength) {
        option.label += "  ✅ Setup";
        configuredProviders.push(providerNameFromProviderEm(option.value));
      }
    }
  }

  const promptText = configuredProviders.length > 0
