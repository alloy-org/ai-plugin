...
function includingFallbackModels(plugin, app, candidateAiModels) {
  if (candidateAiModels.length) { // Augment the user preference with other models available per plugin.ollamaModelsFound & presence of OpenAI key
    if (app.settings[OPENAI_KEY_LABEL]?.length && !candidateAiModels.includes(m => m === DEFAULT_OPENAI_MODEL)) {
    ...
    candidateAiModels = defaultModelCandidates(plugin, app);
  }
