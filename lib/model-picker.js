...
export async function sendQuery(plugin, app, promptKey, promptParams, { contentIndex = null, preferredModels = null,
  ...
  for (let i = 0; i < preferredModels.length; i++) {
    ...
    try {
      response = await responseFromPrompts(plugin, app, aiModel, messages, { allowResponse });
    } catch(e) {
  ...
  return { textResponse: null, modelUsed: null };
}

// --------------------------------------------------------------------------
export function responseFromPrompts(plugin, app, aiModel, messages, { allowResponse = null } = {}) {
  if (isModelOllama(aiModel)) {
    return callOllama(plugin, app, aiModel, messages);
  } else {
    return callOpenAI(plugin, app, aiModel, messages, { allowResponse });
  }
}
...
function includingFallbackModels(plugin, app, candidateAiModels) {
  const startModels = candidateAiModels;
  ...
  console.debug("Ended with", candidateAiModels, "after starting with", startModels);
