...
const plugin = {
  ...
  async _wordReplacer(app, text, promptKey) {
    ...
    const response = await this._notePromptResponse(app, noteUUID, promptKey, { text },
      { contentIndex });
    let options;
    if (response?.result) {
  ...
  async _notePromptResponse(app, noteUUID, promptKey, promptParams, { preferredModels = null, confirmInsert = true,
    ...
    const { response, modelUsed } = await sendQuery(this, app, promptKey, { ...promptParams, noteContent },
