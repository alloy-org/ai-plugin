...
import { APP_OPTION_VALUE_USE_PROMPT, QUESTION_ANSWER_PROMPT } from "constants/prompt-strings.js"
import { LOOK_UP_OLLAMA_MODEL_ACTION_LABEL, OLLAMA_URL, OPENAI_KEY_LABEL } from "constants/provider"
import { AI_MODEL_LABEL, PLUGIN_NAME } from "constants/settings"
...
const plugin = {
  // --------------------------------------------------------------------------------------
  constants: {
    generatedImageCount: app => (app.settings["Images generated per prompt (default 3, more images = longer response time)"]?.trim() || DEFAULT_GENERATED_IMAGE_COUNT),
    imageSize: app => (app.settings["Image size (default is 512, can be 256 or 1024)"]?.trim() || DEFAULT_IMAGE_PIXELS_PER_SIDE),
