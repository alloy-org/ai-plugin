...
const plugin = {
  ...
  replaceText: {
    "Complete": async function(app, text) {
      const result = await this._buildMessagesAndCallOpenAI(app, "complete", text);
      if (result === null) return null;

      const refinedResult = await this._trimNoteContentFromAnswer(app, result, { replaceToken: text });
    ...
    "Revise": async function(app, text) {
      ...
      const result = await this._buildMessagesAndCallOpenAI(app, "reviseText", [ instruction, text ]);
      if (result === null) return null;
  ...
  async _noteOptionInsertContent(app, noteUUID, promptKey, { instruction = null, text = null } = {}, confirmInsert = null) {
    ...
    const result = await this._buildMessagesAndCallOpenAI(app, promptKey, { instruction, noteContent, text });
  ...
  // --------------------------------------------------------------------------
  async _buildMessagesAndCallOpenAI(app, promptKey, promptParams = {}) {
    const messages = this._buildOpenAIMessages(app, promptKey, promptParams);
    const settingModel = app.settings[this.constants.labelOpenAiModel];
    return await callOpenAI(app, messages, settingModel, app.settings[this.constants.labelApiKey].trim(), this.constants.requestTimeoutSeconds);
  },

  // --------------------------------------------------------------------------
  // `promptParams` is an object consisting of `noteContent` key and an optional `instructions` key
  _buildOpenAIMessages(app, promptType, promptParams) {
    let messages = [];
