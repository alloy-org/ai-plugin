import { arrayFromJumbleResponse, cleanTextFromAnswer, optionWithoutPrefix, trimNoteContentFromAnswer } from "app-util"
import {
...
import {
  ADD_PROVIDER_API_KEY_LABEL,
  AI_MODEL_LABEL,
  IMAGE_FROM_PRECEDING_LABEL,
  IMAGE_FROM_PROMPT_LABEL,
  PLUGIN_NAME,
  SEARCH_USING_AGENT_LABEL,
  SUGGEST_TASKS_LABEL,
...
import { aiModelFromUserIntervention, notePromptResponse, recommendedAiModels, sendQuery } from "model-picker"
import { apiKeyFromApp, apiKeyFromAppOrUser, preferredModel, providerFromModel } from "providers/ai-provider-settings"
...
import renderPluginEmbed from "render-embed"
...
const plugin = {
  // --------------------------------------------------------------------------------------
  constants: {
    labelApiKey: null, // Todo: Deprecate PROVIDER_SETTING_KEY_LABELS["openai"],
  ...
  appOption: {
    ...
    [ SEARCH_USING_AGENT_LABEL ]: async function(app) {
      ...
      this.progressText = ""; // Reset progress text
      ...
      searchAgent.emitProgress(`Starting search for user query: "${ userQuery }" with ${ preferredAiModel ? `preferred AI model "${ preferredAiModel }"` : "no preferred AI model" }. ` +
        (changedSince ? `Filtering to notes changed since ${ changedSince }. ` : "") +
        (onlyTags?.length ? `Filtering to notes with tags: ${ onlyTags.join(", ") }. ` : "") +
        (maxNotesCount ? `Limiting to ${ maxNotesCount } notes. ` : `Returning up to ${ DEFAULT_SEARCH_NOTES_RETURNED } notes. `));
      await app.openEmbed();
      await searchAgent.search(userQuery, { options: { dateFilter: { after: changedSince },
        resultCount: maxNotesCount, tagRequirement: { mustHave: onlyTags } }});
    },
    ...
    "Answer": async function(app) {
      ...
      if (userPickedModel) aiModels = [ userPickedModel ].concat(aiModels.filter(model => model !== userPickedModel));
  ...
  // @returns {string|null} The current progress text, or null for unknown actions
  ...
  async _wordReplacer(app, text, promptKey) {
    ...
    if (optionList?.length) {
      ...
      const followUp = apiKeyFromApp(app, providerEm)?.length
        ? `Consider adding ${ providerEm } API key to your plugin settings?`
        : "Try again?";
