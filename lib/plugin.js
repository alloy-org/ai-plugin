import { promptsFromPromptKey, buildMessagesAndCallOpenAI, callOpenAI } from "./plugin-fetch.js"
...
const plugin = {
  ...
  replaceText: {
    "Answer": {
      ...
      async run(app, text) {
        const messages = promptsFromPromptKey("answerSelection", { selectionText: text });
        const result = await callOpenAI(this, app, messages);

        app.alert(`You posed a question: ${ text }`);
      }
    },

    // --------------------------------------------------------------------------
    "Complete": async function(app, text) {
      return buildMessagesAndCallOpenAI(this, app, "replaceTextComplete", { text: `${ text }<token>` })
    },
  ...
  async _noteOptionInsertContent(app, noteUUID, promptKey, { confirmInsert = false,
      instruction = null, rejectedResponses = null, text = null } = {}) {
    const note = await app.notes.find(noteUUID);
    const noteContent = await note.content();
