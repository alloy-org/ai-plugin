import { requestWithRetry } from "./plugin-fetch.js"
...
const plugin = {
  ...
  async _callOpenAI(app, promptType, promptParams) {
    ...
    try {
      ...
      const model = settingModel?.trim()?.length ? settingModel : "gpt-3.5-turbo";
      return await requestWithRetry(app, model, messages, app.settings[this.constants.labelApiKey].trim(),
        { timeout: this.constants.requestTimeoutSeconds });
    } catch (error) {
