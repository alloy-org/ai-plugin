...
const plugin = {
  ...
  noteOption: {
    ...
    "Answer": async function(app, noteUUID) {
      ...
      await this._noteOptionInsertContent(app, noteUUID, "answer", { instruction });
    },
    ...
    "Revise": async function(app, noteUUID) {
      ...
      await this._noteOptionInsertContent(app, noteUUID, "reviseContent", { instruction });
    },
  ...
  async _noteOptionInsertContent(app, noteUUID, promptKey, promptParams, { preferredModels = null, confirmInsert = true,
    ...
    const { result, modelUsed } = await sendQuery(this, app, promptKey, { ...promptParams, noteContent },
    ...
    if (confirmInsert) {
      ...
      preferredModels.forEach(model => {
        actions.push({ icon: "settings", label: `Try ${ model }`, value: model });
      });
      actions.push({ icon: "post_add", label: "Accept content", value: "accept" });

      const selectedValue = await app.alert(result, { actions });
      if (selectedValue === "accept") {
        note.insertContent(result);
      } else if (preferredModels.includes(selectedValue)) {
        const updatedRejects = (rejectedResponses || []).push(result);
        preferredModels = [ selectedValue, ...preferredModels.filter(model => model !== selectedValue) ];
      ...
      app.alert(`Finished generating ${ result.length } character response with ${ modelUsed } in ${ secondsUsed } second${ secondsUsed === 1 ? "" : "s" }`)
      return result;
    }
  },
