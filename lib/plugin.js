...
const plugin = {
  ...
  replaceText: {
    ...
    "Complete": async function(app, text) {
      const { textResponse } = sendQuery(this, app, "replaceTextComplete", { text: `${ text }<token>` });
      return textResponse;
    },
  ...
  async _noteOptionInsertContent(app, noteUUID, promptKey, promptParams, { preferredModels = null, confirmInsert = true,
    ...
    if (confirmInsert) {
      ...
      actions.push({ icon: "post_add", label: "Accept content" });

      const selectedValue = await app.alert(textResponse, { actions });
      console.log("User chose", selectedValue, "from", actions);
      if (selectedValue === preferredModels.length + 1) {
        note.insertContent(textResponse);
      } else if (preferredModels[selectedValue]) {
        const preferredModel = preferredModels[selectedValue];
        const updatedRejects = (rejectedResponses || []).push(textResponse);
