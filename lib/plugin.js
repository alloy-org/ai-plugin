import { buildPromptsAndCallOpenAI } from "./plugin-prompts.js"
...
const plugin = {
  ...
  replaceText: {
    ...
    "Complete": async function(app, text) {
      return buildPromptsAndCallOpenAI(this, app, "replaceTextComplete", { text: `${ text }<token>` })
    },
    ...
    "Revise": async function(app, text) {
      ...
      const result = await buildPromptsAndCallOpenAI(this, app, "reviseText", { instruction, text });
  ...
  async _noteOptionInsertContent(app, noteUUID, promptKey, promptParams,
      { confirmInsert = false, rejectedResponses = null } = {}) {
    const note = await app.notes.find(noteUUID);
    const noteContent = await note.content();

    const result = await buildPromptsAndCallOpenAI(this, app, promptKey, { ...promptParams, noteContent }, rejectedResponses);
    ...
    if (confirmInsert) {
      ...
      if (actionIndex === 0) {
        ...
        return await this._noteOptionInsertContent(app, noteUUID, promptKey, promptParams, { confirmInsert, rejectedResponses: updatedRejects });
