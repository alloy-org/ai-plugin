...
const plugin = {
  ...
  replaceText: {
    ...
    "Complete": async function(app, text) {
      const messages = this._buildOpenAIMessages("replaceTextComplete", { text: `${ text }<token>` });
      const result = await callOpenAI(app, messages, this);
      return result;
    },
  ...
  async _buildMessagesAndCallOpenAI(app, promptKey, promptParams = {}) {
    const messages = this._buildOpenAIMessages(promptKey, promptParams);
    return await callOpenAI(app, messages, this);
  },
