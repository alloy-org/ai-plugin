...
export function pluralize(number, noun) {
  ...
  const numberPart = omitNumber ? "" : `${ number.toLocaleString() } `;
  if (number === 1) {
    return `${ numberPart }${ noun }`;
  } else {
    // This pluralization scheme is woefully minimal atm, but could very easily be expanded as needed
    return `${ numberPart }${ noun }${ parseInt(numberPart) === 1 ? "" : "s" }`;
  }
}
