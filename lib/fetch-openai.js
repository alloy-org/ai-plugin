...
import { responseFromStreamOrChunk, shouldStream } from "./fetch-json"
...
export async function callOpenAI(plugin, app, model, messages, promptKey, allowResponse) {
  model = model?.trim()?.length ? model : DEFAULT_OPENAI_MODEL;

  const streamCallback = shouldStream(plugin) ? streamAccumulate : null;
  try {
