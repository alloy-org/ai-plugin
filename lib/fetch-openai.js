...
export async function callOpenAI(plugin, app, model, messages, jsonEndpoint, { allowResponse = null } = {}) {
  model = model?.trim()?.length ? model : DEFAULT_OPENAI_MODEL;

  const stream = shouldStream(plugin);
  try {
    return await requestWithRetry(app, model, messages, apiKeyFromApp(plugin, app), jsonEndpoint,
...
// --------------------------------------------------------------------------
async function requestWithRetry(app, model, messages, apiKey, jsonResponseExpected, { allowResponse = null, retries = 3, stream = null,
    timeoutSeconds = 30 } = {}) {
  let error, response;
  ...
  if (response?.ok) {
    ...
    if (stream) {
      ...
      result = await responseTextFromStreamResponse(app, response, jsonResponseExpected, streamCallback);
      content = result;
