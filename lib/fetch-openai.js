...
export async function callOpenAI(plugin, app, model, messages, { allowResponse = null } = {}) {
  ...
  try {
    return await requestWithRetry(app, model, messages, apiKeyFromApp(plugin, app),
      { timeoutSeconds: plugin.constants.requestTimeoutSeconds, allowResponse });
  } catch (error) {
...
// --------------------------------------------------------------------------
async function requestWithRetry(app, model, messages, apiKey, { allowResponse = null, retries = 3, timeoutSeconds = 30 } = {}) {
  let error, response;
  ...
  for (let i = 0; i < retries; i++) {
    try {
      console.log("Sending OpenAI query at", new Date());
