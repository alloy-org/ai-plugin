  ...
  if (response?.ok) {
    ...
    if (stream) {
      const streamCallback = decodedValue => {
        ...
        const jsonStart = decodedValue.indexOf("{"); // As of Dec 2023, WBH observes that OpenAI is fond of sending back strings like "data: {\"choices\":[{\"finish_reason\":\"length\"}]}\n\n"
        const json = JSON.parse(decodedValue.substring(jsonStart.trim()));
