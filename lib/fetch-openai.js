...
export async function callOpenAI(plugin, app, model, messages, promptKey, { allowResponse = null } = {}) {
  ...
  try {
    return await requestWithRetry(app, model, messages, apiKeyFromApp(plugin, app), promptKey, jsonEndpoint,
...
// --------------------------------------------------------------------------
async function requestWithRetry(app, model, messages, apiKey, promptKey, jsonResponseExpected, {
