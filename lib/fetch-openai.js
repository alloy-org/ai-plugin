...
async function requestWithRetry(app, model, messages, apiKey, { allowResponse = null, retries = 3, stream = null,
  ...
  if (response?.ok) {
    let result = "";
    if (stream) {
      const streamCallback = (app, decodedValue) => {
        ...
        for (const jsonString of responses) {
          ...
          result += content;
          console.debug("Received content", content, "so now", result, "from", jsonString, "with start", jsonStart, "and json", json, "and app", app);
          app.alert(result, {
            actions: [{
              icon: "pending",
              label: "Cancel generating response",
              value: "cancel"
            }], scrollToEnd: true
          });
        }
