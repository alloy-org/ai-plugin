...
function streamAccumulate(modelsQueriedArray, promptKey, app, decodedValue, receivedContent, aiModel,
  ...
  for (const jsonString of responses) {
    ...
    ({ failedParseContent, jsonResponse, receivedContent } = jsonResponseFromStreamChunk(jsonString.substring(jsonStart).trim(), failedParseContent, receivedContent));
  ...
  return { abort: stop, failedParseContent: null, incrementalContents, receivedContent };
}
