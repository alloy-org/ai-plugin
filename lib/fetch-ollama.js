...
function streamAccumulate(modelsQueriedArray, promptKey, app, decodedValue, receivedContent, aiModel,
  ...
  for (const response of responses) {
    ...
    ({ failedParseContent, jsonResponse, receivedContent } = jsonResponseFromStreamChunk(parseableJson, failedParseContent, receivedContent));
    if (failedParseContent || !jsonResponse) {
      if (failedParseContent) console.debug("Attempting to parse yielded failure. Received content so far is", receivedContent, "this stream deduced", responses.length, "responses");
      return { failedParseContent, receivedContent };
    }
  ...
  return { abort: jsonResponse.done, failedParseContent: null, incrementalContents, receivedContent };
}
