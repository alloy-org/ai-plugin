...
export async function callOllama(plugin, app, model, messages) {
  ...
  let responseText = "";
  if (stream)  {
    console.log("Beginning to stream response from Ollama for", model, "and", messages);
    const response = await fetchPromise;
    const reader = response.body.pipeThrough(new TextDecoderStream()).getReader();
    while (true) {
      const { value, done } = await reader.read();
      console.debug("Received", value, "from Ollama");
      if (done) break;
      ...
      try {
        ...
        console.log("Received exception while attempting to parse streamed response from Ollama:");
        console.error(error);
      }
    }
  } else {
    console.log("Waiting for non-streamed Ollama response")
    const json = await fetchPromise.then(response => response.json());
