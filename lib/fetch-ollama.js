...
import { extractJsonFromString, fetchJson, responseTextFromStreamResponse, shouldStream } from "./fetch-json"
import fetch from "isomorphic-fetch"

// --------------------------------------------------------------------------
// Ollama API docs, illustrating params available via fetch: https://github.com/jmorganca/ollama/blob/main/docs/api.md
export async function callOllama(plugin, app, model, messages, jsonEndpoint) {
  const stream = shouldStream(plugin);

  let responseText;
  if (jsonEndpoint) {
    responseText = await responsePromiseFromGenerate(plugin, app, model, messages, stream);
  } else {
    responseText = await responseFromChat(plugin, app, model, messages, stream);
  }
...
async function responsePromiseFromGenerate(plugin, app, model, messages, stream) {
  ...
  if (systemMessage) {
    jsonQuery.system = systemMessage.content;
    messages = messages.filter(message => message.role !== "system");
  }
  jsonQuery.prompt = messages[0].content;
