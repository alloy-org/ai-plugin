...
async function responseFromChat(plugin, app, model, messages, stream) {
  ...
  if (response) {
    return await responseFromStreamOrChunk(plugin, app, response, model, stream, false);
  } else {
...
// --------------------------------------------------------------------------
async function responseFromStreamOrChunk(plugin, app, response, model, stream, responseJsonExpected) {
  let responseContent = "";
  let responseObject;
  if (stream) {
...
async function responsePromiseFromGenerate(plugin, app, model, messages, stream) {
  ...
  return await responseFromStreamOrChunk(plugin, app, response, model, stream, true)
}
